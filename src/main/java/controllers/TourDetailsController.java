...
public class TourDetailsController {
    ...
    private void sendMessage() {
        ...
        if (!userMessage.isEmpty()) {
            ...
            new Thread(() -> {
                try {
                    String botResponse = GeminiChatService.sendMessage(userMessage);
                    chatArea.appendText("🤖 GuideBot: " + botResponse + "\n\n");
                } catch (IOException e) {
                    if (e.getMessage().contains("403")) {
                        chatArea.appendText("❌ Error: Access denied. Please check your API key and permissions.\n");
                    } else if (e.getMessage().contains("400")) {
                        chatArea.appendText("❌ Error: Invalid request. Please check your input.\n");
                    } else {
                        chatArea.appendText("❌ Error: Unable to contact the Gemini API. Please try again later.\n");
                    }
