...
public class AddFlightController {
    ...
    private void handleSave() {
        ...
        if (departure.equalsIgnoreCase(destination)) {
            showAlert("Erreur de validation", "Le départ et la destination ne peuvent être identiques.");
            return;
        }
        ...
        if (!departureDateTime.isBefore(arrivalDateTime)) {
            showAlert("Erreur de validation", "La date et l'heure de départ doivent être antérieures à la date et à l'heure d'arrivée.");
            return;
        }
        ...
        try {
            ...
            if (price <= 0) {
                showAlert("Erreur de validation", "Le prix doit être un nombre positif.");
                return;
            }
        } catch (NumberFormatException e) {
            showAlert("Erreur de validation", "Prix non valide. Veuillez saisir un numéro valide.");
            return;
        }
        ...
        try {
            ...
            if (numberPlace <= 0) {
                showAlert("Erreur de validation", "Le nombre de places doit être un nombre entier positif.");
                return;
            }
        } catch (NumberFormatException e) {
            showAlert("Erreur de validation", "Nombre de places non valide. Veuillez saisir un nombre valide.");
            return;
        }

        // Validate flight number format (Example: "AA123", 2 letters followed by digits)
        if (!flightNumber.matches("^[A-Z]{2}\\d{2,4}$")) {
            showAlert("Erreur de validation", "Format du numéro de vol non valide. Exemple : AA123");
            return;
        }
        ...
        if (departureAirport == null || arrivalAirport == null) {
            showAlert("Erreur de validation", "Sélection d'aéroport non valide.");
            return;
        }
        ...
        try {
            ...
            showAlert("Erreur de base de données", "Une erreur s'est produite lors de l'enregistrement du vol.");
            e.printStackTrace();
        }
    }
