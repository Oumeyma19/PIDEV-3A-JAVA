...
public class AjouterOffre {
    ...
    private void chooseImage() {
        ...
        if (selectedFile != null) {
            String sharedDirPath = "C:/Users/moham/Downloads/PIDEV-3A-Symfony--main (2)/PIDEV-3A-Symfony--main/public/uploads/";
            String newFileName = UUID.randomUUID().toString() + getFileExtension(selectedFile);
            File destFile = new File(sharedDirPath + newFileName);

            try {
                Files.copy(selectedFile.toPath(), destFile.toPath(), StandardCopyOption.REPLACE_EXISTING);
                // Save 'uploads/newFileName' into the database
                String dbPath = "uploads/" + newFileName;
                this.imagePath = dbPath;

                // Preview image in JavaFX (optional)
                imageView.setImage(new Image(destFile.toURI().toString()));

                // Save dbPath to your entity or send to backend here

            } catch (IOException e) {
                showAlert("Error", "Failed to copy image: " + e.getMessage());
            }
        } else {
    ...
    private void ajouterOffre() {
        ...
        String start_date = (startDatePicker.getValue() != null) ? startDatePicker.getValue().toString() : null;
        String end_date = (endDatePicker.getValue() != null) ? endDatePicker.getValue().toString() : null;

        // Validation
        if (title.isEmpty() || description.isEmpty() || start_date == null || end_date == null || imagePath == null) {
