    ...
    private void redirectToHome(User user) {
        try {
            ...
            if (user.getRoles() == Type.ADMIN) {
                loader = new FXMLLoader(getClass().getResource("/views/Dhashboard_tours_view.fxml"));
            } else {
            ...
            if (user.getRoles() == Type.ADMIN) {
                DashboardToursController DashboardToursController = loader.getController();
                DashboardToursController.setCurrentUser(user);
            } else {
