...
public class ProfilController {
    ...
    public void handleOffre(MouseEvent mouseEvent) {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/views/reservation_list.fxml"));
            Parent root = loader.load();
            ...
            // Pass the current user to the reservations controller
            ReservationListController.setCurrentUser(currentUser);
    ...
    @FXML
    private void handleRecompenseClick(javafx.scene.input.MouseEvent event) {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/views/UserRecompense.fxml"));
            Parent root = loader.load();

            UserRecompenseController userRecompenseController = loader.getController();
            userRecompenseController.setCurrentUser(currentUser);

            Stage stage = (Stage) ((Node) event.getSource()).getScene().getWindow();
            stage.setScene(new Scene(root));
            stage.show();
        } catch (IOException e) {
            System.err.println("Erreur lors de la redirection vers la page des récompenses : " + e.getMessage());
        }
    }
}
