...
public class ReservationOffreService implements IService<ReservationOffre> {
    ...
    public void ajouter(ReservationOffre r) throws SQLException {
        String sql = "INSERT INTO reservation_offres (offer_id, start_Date, end_Date, status, id_user, number_of_adults, number_of_children) " +
                "VALUES (?, ?, ?, ?, ?, ?, ?)";

        try (PreparedStatement stmt = cnx.prepareStatement(sql)) {
            stmt.setInt(1, r.getOffre().getId());  // Offer ID
            stmt.setDate(2, Date.valueOf(r.getstart_date()));  // Start date
            stmt.setDate(3, Date.valueOf(r.getend_date()));    // End date
    ...
    public void modifier(ReservationOffre r) throws SQLException {
        String sql = "UPDATE reservation_offres SET offer_id = ?, start_date = ?, end_date = ?, status = ?, id_user = ?, " +
        ...
        try (PreparedStatement stmt = cnx.prepareStatement(sql)) {
            ...
            while (rs.next()) {
                ReservationOffre r = new ReservationOffre(
                        ...
                        new User(rs.getInt("id_user"), rs.getString("firstname"), rs.getString("lastname"), rs.getString("email"), rs.getString("phone"), rs.getString("password"),rs.getInt("pointsfid"), rs.getInt("nivfid"), Type.CLIENT, rs.getBoolean("is_banned"), rs.getBoolean("is_active")), // User
