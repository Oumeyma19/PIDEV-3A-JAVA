...
public class OffreService implements IService<Offre> {
    ...
    @Override
    public void modifier(Offre p, String nom) throws SQLException {
    ...
    public List<Offre> recuperer() throws SQLException {
        ...
        while (rs.next()) {
            Offre o = new Offre();
            o.setId(rs.getInt("id")); // Fix: Set the ID before fetching related data
            ...
            String sqlH = "SELECT h.* FROM hebergements h JOIN offre_hebergement oh ON h.id = oh.hebergement_id WHERE oh.offre_id = ?";
            try (PreparedStatement stH = cnx.prepareStatement(sqlH)) {
                stH.setInt(1, o.getId());
                ResultSet rsH = stH.executeQuery();
                while (rsH.next()) {
                    Hebergements h = new Hebergements();
                    h.setIdHebrg(rsH.getInt("id"));
                    h.setNomHeberg(rsH.getString("nom_hebrg"));
                    hebergements.add(h);
                }
            }
            ...
            try (PreparedStatement stT = cnx.prepareStatement(sqlT)) {
            ...
            String sqlF = "SELECT f.* FROM flight f JOIN offre_flight ofl ON f.id = ofl.flight_id WHERE ofl.offre_id = ?";
            try (PreparedStatement stF = cnx.prepareStatement(sqlF)) {
                stF.setInt(1, o.getId());
                ResultSet rsF = stF.executeQuery();
                while (rsF.next()) {
                    Flight f = new Flight();
                    f.setIdFlight(rsF.getInt("id"));
                    f.setFlightNumber(rsF.getString("flight_number"));
                    f.setDeparture(rsF.getString("departureTime"));
                    flights.add(f);
                }
            }
