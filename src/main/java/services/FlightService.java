...
public class FlightService implements IService<Flight> {
    ...
    public void ajouter(Flight p) throws SQLException {
        String sql = "INSERT INTO flight (id, departure, destination, departureTime, arrivalTime, price, flight_number, departure_Airport_id, arrival_Airport_id , numbre_place) " +
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?,?)";
    ...
    public Flight getFlightById(int id) {
        Flight flight = null;
        String sql = "SELECT f.*, da.id AS dep_airport_id, da.nameAirport AS dep_airport_name, " +
                "aa.id AS arr_airport_id, aa.nameAirport AS arr_airport_name " +
                "FROM flight f " +
        ...
        try (PreparedStatement pst = cnx.prepareStatement(sql)) {
            ...
            System.err.println("Error retrieving flight with ID " + id + ": " + e.getMessage());
            e.printStackTrace();
        }
