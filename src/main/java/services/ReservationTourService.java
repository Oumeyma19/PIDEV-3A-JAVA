...
public class ReservationTourService {
    ...
    public void addReservation(ReservationTour reservation) {
        try {
            ...
            ps.setDate(4, new Date(reservation.getReservationDate().getTime())); // Set reservation date
            ps.executeUpdate();
    ...
    public List<ReservationTour> getAllReservations() {
        List<ReservationTour> reservations = new ArrayList<>();
        try {
            Statement stmt = connection.createStatement();
            ResultSet rs = stmt.executeQuery("SELECT * FROM reservationstours");
            while (rs.next()) {
                reservations.add(new ReservationTour(
                        rs.getInt("id"),
                        rs.getInt("client_id"),
                        rs.getInt("tour_id"),
                        rs.getString("status"),
    ...
    public List<ReservationTour> getReservationsByTourId(int tourId) throws SQLException {
        List<ReservationTour> reservations = new ArrayList<>();
    ...
    public List<ReservationTour> getReservationsByUserId(int userId) throws SQLException {
        List<ReservationTour> reservations = new ArrayList<>();
        String sql = "SELECT * FROM reservationstours WHERE client_id = ?";
        PreparedStatement ps = connection.prepareStatement(sql);
        ps.setInt(1, userId);
        ResultSet rs = ps.executeQuery();
