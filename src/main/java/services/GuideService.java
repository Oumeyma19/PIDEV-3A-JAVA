...
public class GuideService implements UserInterface {
    ...
    public void addUser(User user) throws EmptyFieldException, InvalidPhoneNumberException, InvalidEmailException,
        ...
        String request = "INSERT INTO `user`(`firstname`, `lastname`, `email` ,`phone`,`password`,`status_guide`,`roles`,`is_active`,`is_banned`) " +
            "VALUES (?,?,?,?,?,?,?,?,?)";
        try {
    ...
    public void updateUser(User user) throws EmptyFieldException, InvalidPhoneNumberException, InvalidEmailException, IncorrectPasswordException, UserNotFoundException {
        ...
        String request = "UPDATE user SET firstname = ?, lastname = ?, email = ?, phone = ?, status_guide = ?, is_banned = ?, is_active = ? WHERE id = ?";
        try (PreparedStatement preparedStatement = connection.prepareStatement(request)) {
            preparedStatement.setString(1, user.getFirstname());
            preparedStatement.setString(2, user.getLastname());
            preparedStatement.setString(3, user.getEmail());
            preparedStatement.setString(4, user.getPhone());
            preparedStatement.setBoolean(5, user.getstatus_guide());
        ...
        try {
            ...
            while (resultSet.next()) {
                User user = new User(
                    ...
                    resultSet.getBoolean("status_guide"),
                    Type.GUIDE,
    ...
    private User createUserFromResultSet(ResultSet resultSet) throws SQLException {
        ...
        Boolean status_guide = resultSet.getBoolean("status_guide"); // Nullable
        ...
        return new User(id, firstname, lastname, email, phone, password, status_guide, roles, is_banned,is_active);
    }
