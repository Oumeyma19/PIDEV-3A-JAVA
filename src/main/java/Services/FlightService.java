...
public class FlightService implements IService<Flight> {
    ...
    public void ajouter(Flight p) throws SQLException {
        String sql = "INSERT INTO flight (idFlight, departure, destination, departureTime, arrivalTime, price, flight_number, departure_Airport_id, arrival_Airport_id) " +
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    public void modifier(Flight p, String flightNumber) {
        String sql = "UPDATE flight SET departure = ?, destination = ?, departureTime = ?, arrivalTime = ?, price = ?, flight_number = ?, departure_Airport_id = ?, arrival_Airport_id = ? WHERE flight_number = ?";
        try (PreparedStatement pst = cnx.prepareStatement(sql)) {
